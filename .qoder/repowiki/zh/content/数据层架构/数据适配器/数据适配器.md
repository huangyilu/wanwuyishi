...
## 更新摘要
**所做更改**
- 更新了 Supabase 云端适配器的自定义城市字段支持，包括 customCity、customFromCity、customToCity
- 增强了 TripDay 和 TripItem 类型定义以支持用户自定义城市名称
- 完善了数据库迁移文件中的字段映射和数据持久化逻辑
- 更新了适配器选择逻辑和错误处理机制以支持新的自定义城市功能
...
**更新** Supabase 适配器现已支持自定义城市字段，允许用户输入不在世界库中的城市名称并正确持久化到数据库。
...
**更新** TripDay 和 TripItem 接口现在支持自定义城市字段，允许用户输入不在世界库中的城市名称。
...
章节来源
- [src/data/adapters/local-trip.ts:1-352](file://src/data/adapters/local-trip.ts#L1-L352)
...
**更新** 自定义城市字段支持：
- `customCity`：当 TripDay 的 cityId 为 null 时，存储用户自定义的城市名称
- `customFromCity`：当 TripItem 的 fromCityId 为 null 时，存储出发城市的自定义名称
- `customToCity`：当 TripItem 的 toCityId 为 null 时，存储到达城市的自定义名称
...
Note over Repo,DB : 自定义城市字段映射：custom_city, custom_from_city, custom_to_city
```

**图表来源**
- [src/data/adapters/supabase-trip.ts:159-198](file://src/data/adapters/supabase-trip.ts#L159-L198)
- [src/data/adapters/supabase-trip.ts:171-179](file://src/data/adapters/supabase-trip.ts#L171-179)

章节来源
- [src/data/adapters/supabase-trip.ts:1-557](file://src/data/adapters/supabase-trip.ts#L1-L557)
...
- 云端适配器
  ...
  - **更新** 自定义城市字段的存储和读取增加了少量数据传输开销，但保持了向后兼容性。
- 世界库适配器
...
- 云端适配器
  ...
  - **更新** 自定义城市字段验证：确保 customCity、customFromCity、customToCity 字段正确映射到数据库列。
- 世界库适配器
...
**更新** 最新的自定义城市字段支持进一步增强了用户体验，允许用户输入世界库中不存在的城市名称，同时保持与现有系统的完全兼容性。
...
- 步骤
  ...
  2. 设计数据模型映射
     ...
     - **更新** 对于自定义城市字段，确保正确处理 customCity、customFromCity、customToCity 与数据库列的映射。
  3. 实现核心方法
...
- 注意事项
  ...
  - **更新** 对于自定义城市字段，确保在数据库迁移文件中正确定义列，并在适配器中进行适当的映射和验证。
