...
## 更新摘要
**所做更改**
- 增强了城市选择功能，新增 CityCombo 组件支持灵活的城市输入
- 更新了 TripItem 类型定义，支持自定义城市字段
- 完善了交通和住宿条目的城市选择体验
- 增强了移动端行程显示中的城市信息展示
...
**最新更新**：增强了城市选择功能，新增自定义城市输入支持和智能目的地检测功能，ItemEditor组件现在支持CityCombo组件进行灵活的城市选择。
...
## 核心组件
- ItemEditor：条目编辑面板，支持 POI/交通/住宿/备注四类条目，提供标题、时段、城市、状态、备注、地址、图片附件等字段编辑，并通过 mutation 进行乐观更新。**新增**：使用 CityCombo 组件进行灵活的城市选择。
...
- types：定义 TripItem、Ticket、TripBundle 等核心数据结构。**更新**：TripItem 新增 customFromCity 和 customToCity 字段。
...
IE->>CC : 选择/输入城市
CC->>IE : onCommit(cityId, customCity)
...
- [ItemEditor.tsx:454-495](file://src/features/trip/ItemEditor.tsx#L454-L495)
...
- **新增城市选择功能**：
  - 使用 CityCombo 组件替代传统下拉选择
  - 支持从世界库预设城市中选择或自由输入自定义城市名
  - 智能匹配：输入时自动匹配预设城市的 name 或 localName
  - 失焦提交：避免每次按键都触发 mutation，提升性能
  - 同时支持 fromCityId/customFromCity 和 toCityId/customToCity 字段
- 图片附件：
...
TransFields --> CityCombo["CityCombo 城市选择"]
AccFields --> CityCombo
CityCombo --> Match{"匹配预设城市?"}
Match --> |是| SetCityId["设置 cityId"]
Match --> |否| SetCustom["设置 customCity"]
SetCityId --> Patch["调用 updateItem.patch"]
SetCustom --> Patch
POIFields --> Patch
AccFields --> Patch
NoteFields --> Patch
...
- 功能特性：
  - 支持从世界库预设城市中选择
  - 支持自由输入自定义城市名
  - 智能匹配：同时匹配城市名称（name）和本地名称（localName）
  - 本地缓冲：使用本地 state 缓冲输入过程，失焦时再提交
  - datalist 建议：提供预设城市选项的下拉建议
- 数据绑定：
  - 接收 cityId 和 customCity 作为受控属性
  - 通过 onCommit 回调返回 (cityId, customCity) 元组
  - 空值处理：清空时返回 (null, null)
- 用户体验：
  - 实时显示当前选中的城市名称
  - 支持模糊匹配和智能提示
  - 避免频繁的网络请求，提升响应速度
...
```mermaid
flowchart TD
Input["用户输入"] --> LocalState["本地 state 缓冲"]
LocalState --> Display["显示当前值"]
Display --> Blur{"失焦事件"}
Blur --> Trim["去除空白字符"]
Trim --> Match{"匹配预设城市"}
Match --> |找到| SetCityId["设置 cityId"]
Match --> |未找到| SetCustom["设置 customCity"]
SetCityId --> Commit["onCommit(cityId, null)"]
SetCustom --> Commit
Commit --> Clear["清空本地 state"]
```
...
- 组件耦合：
  ...
  - **新增**：ItemEditor 依赖 CityCombo 组件进行城市选择
- 数据流：
...
- **新增城市选择优化**：
  - CityCombo 组件使用本地 state 缓冲输入，避免每次按键触发 mutation
  - 失焦时才提交变更，减少不必要的网络请求
  - 智能匹配算法同时支持中英文城市名识别
...
- 常见问题定位：
  ...
  - **新增**：城市选择不生效：检查 cities 数据是否正确传递、CityCombo 的 onCommit 回调是否正常工作
- 错误反馈：
...
## 结论
项目编辑器通过分层架构与乐观更新机制，实现了高效、流畅的行程编辑体验。ItemEditor 与 TicketEditor 分别覆盖条目与票务的核心编辑场景，配合 Workbench 与 Timeline 的拖拽排程，形成完整的编辑闭环。**最新更新**：新增的 CityCombo 组件大幅提升了城市选择的灵活性和用户体验，支持自定义城市输入和智能目的地检测。sanity-check 提供第二道防线，帮助用户规避常见规划陷阱。未来可扩展更多自定义编辑器与第三方组件集成。
...
- 步骤：
  ...
  5. **新增**：对于需要城市选择的场景，使用 CityCombo 组件替代传统输入
- 示例路径参考：
  ...
  - 城市选择集成：[ItemEditor.tsx:159-176](file://src/features/trip/ItemEditor.tsx#L159-L176)

### 处理复杂表单
- 使用受控与非受控混合：
  - 简单字段使用受控（如 select/input）
  - 长文本使用非受控（defaultValue + onBlur 触发 patch）
  - **新增**：城市选择使用本地 state 缓冲，失焦时提交
- 字段校验：
  - 前端即时校验（如图片类型/大小）
...
- 示例路径参考：
  ...
  - 城市选择逻辑：[ItemEditor.tsx:470-477](file://src/features/trip/ItemEditor.tsx#L470-L477)
...
- **新增**：城市选择：内置 CityCombo 组件，无需额外依赖
- 示例路径参考：
  ...
  - 城市选择集成：[ItemEditor.tsx:454-495](file://src/features/trip/ItemEditor.tsx#L454-L495)
...
- 业务规则校验：
  ...
  - 示例路径参考：
    ...
    - 城市匹配逻辑：[ItemEditor.tsx:472-475](file://src/features/trip/ItemEditor.tsx#L472-L475)

### 数据绑定与变更追踪
- 双向数据绑定：
  - 受控组件：value + onChange -> patch -> mutate
  - 非受控组件：defaultValue + onBlur -> patch -> mutate
  - **新增**：城市选择：本地 state 缓冲 + 失焦提交 -> patch -> mutate
- 变更追踪：
...
- **新增数据类型支持**：
  - TripItem 新增 customFromCity 和 customToCity 字段
  - 支持同时存储预设城市 ID 和自定义城市名
  - 移动端显示时智能选择显示哪个城市名
