...
## 更新摘要
**变更内容**
- 更新了票务编辑器输入字段的背景样式，使用 `var(--bg-inset)` 替代 `var(--bg-sub)`，在白色卡片容器内创建正确的内陷外观
- 改进了不同背景层级之间的视觉区分度，提升用户体验
- 保持了与整体设计系统的颜色变量一致性
...
## 简介
本文件为"票务编辑器"的完整技术文档，聚焦于行程中的票务信息管理。当前实现以"景点导览卡内嵌的订票信息"为核心，支持创建、编辑、删除单条票务记录；数据通过统一的数据仓库接口（Repository）持久化到本地存储或云端数据库。**最新更新**：已增强PDF附件管理功能，支持上传、查看和管理电子票证PDF文件，提供完整的电子票证生命周期管理能力。同时优化了输入字段的视觉层次，使用恰当的背景色变量提升界面质感。
...
- 样式层：基于设计系统Token的CSS变量，确保一致的视觉体验
...
UI --> Styles["TicketEditor.module.css<br/>设计系统Token"]
Styles --> Tokens["tokens.css<br/>CSS变量定义"]
```
...
- [TicketEditor.module.css:62-78](file://src/features/trip/TicketEditor.module.css#L62-L78)
...
- **样式系统**：基于设计系统Token的CSS变量，确保一致的视觉层次和用户体验
...
- **视觉层次优化**（更新）：
  - 输入字段使用 `var(--bg-inset)` 背景色，在白色卡片容器内呈现内陷效果
  - 提升不同背景层级间的视觉区分度，改善用户体验

**章节来源**
- [TicketEditor.tsx:15-245](file://src/features/trip/TicketEditor.tsx#L15-L245)
- [TicketEditor.module.css:62-78](file://src/features/trip/TicketEditor.module.css#L62-L78)
- [TicketEditor.module.css:120-127](file://src/features/trip/TicketEditor.module.css#L120-L127)

### 视觉设计与样式系统
**更新** 票务编辑器采用了改进的视觉层次设计：

- **背景色系统**：
  - `--bg-sub` (#FCFAF2): 页面整体底色，暖纸米白
  - `--bg` (#ffffff): 卡片/面板背景，纯白色
  - `--bg-inset` (#EDE7D8): 输入框/内陷区域背景，浅米灰色
  - `--surface-2` (#F7F3EA): 白卡内的嵌套面板背景

- **输入字段样式**：
  - 时间输入框、文本输入框、文本域均使用 `var(--bg-inset)` 背景
  - 在白色卡片容器内形成明显的内陷视觉效果
  - 边框使用 `var(--line)` 保持柔和的边界感

- **PDF附件区域**：
  - 附件列表项使用 `var(--bg-inset)` 背景，与输入字段保持一致
  - 悬停状态提供适当的反馈
...
- **样式依赖**：
  - TicketEditor.module.css 依赖 tokens.css 中的CSS变量定义
  - 使用设计系统Token确保一致的视觉体验
...
- **样式显示问题**（更新）：
  - 背景色不正确：检查CSS变量是否正确定义和引入
  - 视觉层次混乱：确认使用了正确的背景色变量（--bg-inset vs --bg-sub）
  - 主题切换异常：验证tokens.css中的变量定义是否完整
...
## 结论
当前票务编辑器围绕"景点导览卡内的订票信息"提供了完整的录入、编辑与删除能力，采用乐观更新提升交互体验，并通过 Repository 抽象同时支持本地与云端持久化。**最新增强**的PDF附件管理功能为用户提供了完整的电子票证管理能力，包括上传、预览、删除等操作，大大提升了用户体验。同时，**样式系统的优化**通过使用恰当的背景色变量（--bg-inset）替代通用的背景色（--bg-sub），在白色卡片容器内创建了正确的内陷外观，显著改善了不同背景层级之间的视觉区分度。数据结构清晰、约束明确，便于后续扩展更多票务类型（机票、火车、船票）、集成第三方 API 与实现提醒功能。导入导出与批量操作可在现有基础上逐步增强。
...
- **设计系统Token规范**（更新）：
  - 背景色层次：--bg-sub (页面底) → --bg (卡片底) → --bg-inset (输入框底)
  - 颜色值：--bg-sub (#FCFAF2), --bg (#ffffff), --bg-inset (#EDE7D8)
  - 使用原则：在白色卡片容器内的输入元素应使用 --bg-inset 创建内陷效果
  - 主题一致性：所有组件都应通过CSS变量而非硬编码颜色实现主题切换
