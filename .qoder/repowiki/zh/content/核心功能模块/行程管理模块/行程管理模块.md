...
## 更新摘要
**所做更改**
- 增强了城市选择灵活性，支持自定义城市输入功能
- Timeline组件的日期卡片增加了内联城市输入功能
- ItemEditor组件实现了智能城市选择器CityCombo
- 更新了数据模型以支持自定义城市存储
- 改进了移动端的城市显示逻辑
...
- **增强的城市选择**：支持从世界库预设城市中选择或自由输入自定义城市名，提供更灵活的目的地管理。
...
- 视图层
  ...
  - 时间线：Timeline.tsx（按天卡片、候选池、条目行、拖拽落点、**内联城市输入**）
  - 条目编辑：ItemEditor.tsx（字段编辑、图片附件、状态切换、**智能城市选择器**）
  - 移动端视图：MobileTrip.tsx（移动端专用行程展示、**自定义城市显示**）
...
- 状态与数据
  ...
  - 数据模型：types.ts（TripBundle、TripItem、Day、成员、票务等、**自定义城市字段**）
...
- [Timeline.tsx:1-723](file://src/features/trip/Timeline.tsx#L1-L723)
- [ItemEditor.tsx:1-495](file://src/features/trip/ItemEditor.tsx#L1-L495)
- [MobileTrip.tsx:1-219](file://src/features/trip/MobileTrip.tsx#L1-L219)
...
- 时间线 Timeline
  ...
  - **新增内联城市输入**：日期卡片头部支持直接输入或选择城市，支持预设城市和自定义城市。
  - 展示状态、投票、票务、闭馆提示、计划时长统计。
- 条目编辑器 ItemEditor
  - 针对不同 kind 暴露字段（时间、城市、地址、备注、状态、图片）。
  - **新增 CityCombo 组件**：智能城市选择器，支持从世界库选择或自由输入自定义城市名。
  - 支持地址复制、图片上传（云端）、状态切换、共享备注。
  - **新增"把当天城市设为此处"功能**：快速设置日期卡片的城市。
- 移动端 MobileTrip
  - 专为移动端设计的单列行程展示界面。
  - 支持日期切换、闭馆提醒、条目详情查看。
  - **改进的城市显示**：支持显示自定义城市名称，交通路线显示更准确。
  - 集成下拉菜单系统优化操作体验。
- 移动端今日视图 MobileToday
...
Q-->>TL : onSettled 失效缓存
U->>TL : 在日期卡片中输入城市
TL->>TL : commitCity 处理城市输入
TL->>Q : updateDay.mutate({cityId/customCity})
Q->>R : repo.updateDay(...)
R-->>Q : 返回更新结果
```
...
- [Timeline.tsx:353-370](file://src/features/trip/Timeline.tsx#L353-L370)
...
**新增功能**：系统现在支持灵活的城市选择，既可以从世界库预设城市中选择，也可以自由输入自定义城市名。

#### 城市选择器架构
- **CityCombo 组件**：智能城市选择器，提供统一的输入体验
  - 支持从 datalist 中选择预设城市
  - 支持自由输入自定义城市名
  - 失焦时提交，避免频繁的网络请求
  - 智能匹配：优先匹配预设城市，否则保存为自定义文本
...
// TripDay 类型扩展
interface TripDay {
  cityId: string | null;        // 预设城市ID
  customCity: string | null;    // 自定义城市名
  // ... 其他字段
}

// TripItem 类型扩展  
interface TripItem {
  fromCityId?: string | null;   // 出发城市ID
  toCityId?: string | null;     // 到达城市ID
  customFromCity?: string | null; // 自定义出发城市
  customToCity?: string | null;   // 自定义到达城市
  // ... 其他字段
}
```

#### 城市显示逻辑
- **优先级规则**：预设城市名 > 自定义城市名 > 空值
- **兼容性处理**：旧数据中只有 cityId 的情况正常显示
- **移动端优化**：简化显示逻辑，确保在小屏幕上正确显示

**章节来源**
- [Timeline.tsx:353-401](file://src/features/trip/Timeline.tsx#L353-L401)
- [ItemEditor.tsx:449-495](file://src/features/trip/ItemEditor.tsx#L449-L495)
- [types.ts:151-192](file://src/data/types.ts#L151-L192)
- [MobileTrip.tsx:146-153](file://src/features/trip/MobileTrip.tsx#L146-L153)
...
- 编辑详情
  ...
  - **新增智能城市选择**：CityCombo 组件提供统一的城市输入体验。
...
- 组件耦合
  ...
  - Timeline 依赖 dnd-kit、rank、sanity-check、ItemRow（含投票、票务、状态）、**CityCombo**。
  - ItemEditor 依赖 uploadAttachment、CopyButton、状态选择、**CityCombo**。
  - **移动端组件**：MobileTrip 依赖 MobileToday、PoiGuideCard、状态管理、**改进的城市显示逻辑**。
- 数据流
...
- [queries.ts:1-236](file://src/features/trip/queries.ts#L1-L236)
- [local-trip.ts:1-349](file://src/data/adapters/local-trip.ts#L1-L349)
- [types.ts:1-315](file://src/data/types.ts#L1-L315)
...
- 渲染优化
  ...
  - **城市输入优化**：使用本地 state 缓冲输入过程，失焦时再提交，避免每次按键都打一次 mutation。
- **移动端性能优化**
  - 组件懒加载：移动端和工作台分别懒加载，减少初始包体积
  - 内存优化：移动端使用更简洁的数据结构和渲染逻辑
  - 网络优化：移动端优先使用本地缓存，减少网络请求
  - **城市显示优化**：简化的城市显示逻辑，减少不必要的查找操作
...
- **城市输入问题**
  - 现象：城市输入不生效或显示不正确。
  - 排查：检查 CityCombo 组件的 onCommit 回调；确认 cityId 和 customCity 字段的正确设置；验证 datalist 选项是否正确加载。
  - 参考路径：[Timeline.tsx:353-370](file://src/features/trip/Timeline.tsx#L353-L370)、[ItemEditor.tsx:449-495](file://src/features/trip/ItemEditor.tsx#L449-L495)
...
## 结论
本模块通过清晰的三栏架构、分数序排序算法与统一的乐观更新机制，实现了高效、可扩展的行程管理体验。作为 **Tour the World** 旅行作战台的核心功能，它既能满足个人规划，也支持团队协作。**最新的城市选择功能增强提供了更大的灵活性，用户既可以选择世界库中的预设城市，也可以自由输入自定义城市名，大大提升了行程规划的便利性**。Timeline组件的内联城市输入和ItemEditor的智能城市选择器为用户提供了流畅的操作体验。移动端的城市显示逻辑也得到了改进，确保在各种设备上都能正确显示城市信息。未来可进一步扩展云端协作、智能路线优化与更丰富的可视化能力。
...
- **更新日期城市（updateDay）**
  - 参数：{ id, patch: { cityId?, customCity? } }
  - 行为：更新日期的城市信息，支持预设城市ID或自定义城市名。
  - 返回：TripDay
  - 参考路径：[local-trip.ts:180-188](file://src/data/adapters/local-trip.ts#L180-L188)、[queries.ts:218-221](file://src/features/trip/queries.ts#L218-L221)
- 合理性体检（sanityCheck）
...
- [local-trip.ts:180-253](file://src/data/adapters/local-trip.ts#L180-L253)
- [queries.ts:83-130](file://src/features/trip/queries.ts#L83-L130)
- [queries.ts:218-221](file://src/features/trip/queries.ts#L218-L221)
- [sanity-check.ts:84-176](file://src/domain/trip/sanity-check.ts#L84-L176)