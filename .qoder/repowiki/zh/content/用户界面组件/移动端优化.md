...
B --> G["useViewMode.ts<br/>模式状态源"]
E --> H["today-hero.ts<br/>今日推导纯函数"]
D --> I["AppShell.module.css<br/>底部 Tab 与头像菜单"]
E --> J["MobileTrip.module.css<br/>卡片/Sheet/滚动"]
A --> K["base.css<br/>全局与移动端覆盖"]
```
...
## 结论
该应用在移动端通过“非响应式”的双端分流，实现了专注的随身模式：单列卡片、底部 Tab、底部 Sheet 详情、安全区域适配与触控优化，兼顾了信息密度与易用性。同时，通过纯函数化的“今日推导”与模块化样式覆盖，保证了可测试性与一致性。建议在后续迭代中继续强化首包体积监控、懒加载与离线缓存策略，进一步提升移动端性能与稳定性。
