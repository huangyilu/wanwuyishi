...
## 结论
PdfViewer 组件以最小依赖实现了可靠的 PDF 预览能力，兼容 Supabase 存储与直链场景，具备良好的用户体验与内存管理。结合 TicketEditor 的使用方式，可在旅行票据场景中便捷地上传与预览附件。后续可考虑增加缓存、重试与更丰富的错误提示以提升健壮性。