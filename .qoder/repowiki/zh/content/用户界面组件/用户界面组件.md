...
## 更新摘要
**变更内容**
- 将品牌名称从"玩无一失"更新为"Tour the World"
- 更新所有相关的UI文本和标识
- 保持文档结构和技术内容的一致性
...
## 简介
本文件系统化梳理 Tour the World 应用中的可复用 UI 组件与样式体系，覆盖设计原则、组件层次、样式策略、响应式实现、Props 接口、事件处理、状态管理、主题定制、使用示例、最佳实践与性能优化。重点说明 CSS Modules 的使用方式与样式组织方法，帮助读者在不深入源码的情况下也能快速理解并正确使用这些组件。
...
**图表来源**
- [AppShell.tsx:1-239](file://src/app/AppShell.tsx#L1-L239)
...
- 参考路径：
  - [AppShell.tsx:212-239](file://src/app/AppShell.tsx#L212-L239)
  - [useViewMode.ts:1-63](file://src/hooks/useViewMode.ts#L1-L63)
...
### 品牌标识更新
**更新** 应用已完成品牌重命名，从"玩无一失"更名为"Tour the World"

- 应用标题与描述
  - HTML 标题：Tour the World
  - 元描述：Tour the World —— 把散落各处的攻略串成一份可执行的行程
  - 启动画面：Tour the World · 加载中
  - 参考路径：[index.html:10-15](file://index.html#L10-L15)

- 应用外壳品牌标识
  - 桌面端品牌：Tour the World + Plan. Pack. Go.
  - 移动端导航：保持一致的品牌展示
  - 参考路径：[AppShell.tsx:41-51](file://src/app/AppShell.tsx#L41-L51)

- 页面品牌展示
  - 行程页英雄区：Tour the World 品牌标识
  - 参考路径：[TripsPage.tsx:55-58](file://src/pages/TripsPage.tsx#L55-L58)

- AI 助手品牌
  - 对话引擎标识：「Tour the World」旅行行程助手
  - 参考路径：[ChatPanel.tsx:423](file://src/features/trip/ChatPanel.tsx#L423)

- 技术文档品牌
  - 世界库 schema 注释：Tour the World · 世界库 schema
  - 参考路径：[schema.ts:1-2](file://src/domain/world/schema.ts#L1-L2)

**章节来源**
- [index.html:10-15](file://index.html#L10-L15)
- [AppShell.tsx:41-51](file://src/app/AppShell.tsx#L41-L51)
- [TripsPage.tsx:55-58](file://src/pages/TripsPage.tsx#L55-L58)
- [ChatPanel.tsx:423](file://src/features/trip/ChatPanel.tsx#L423)
- [schema.ts:1-2](file://src/domain/world/schema.ts#L1-L2)